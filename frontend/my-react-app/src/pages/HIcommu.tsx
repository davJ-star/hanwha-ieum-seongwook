...
const PostList = ({ posts = [], categories, navigate, onSearch, currentPage, totalPages, onPageChange }: 
  ...
  <section className="post-list-container" aria-labelledby="postListTitle">
    ...
    <header className="post-header">
      <h2 id="postListTitle">청각장애 게시판</h2>
      <button 
...
const CommunityMain = () => {
  ...
  const fetchCommunityData = (page: number = 0) => {
    // 페이지 네이션 추가
    // 게시글 목록 조회 API 호출
    axios.get(`http://13.124.88.193:8080/community/AUDITORY?page=${page}&size=9`)
  ...
  const handleSearch = (keyword: string) => {
    ...
    axios.get(`http://13.124.88.193:8080/community/AUDITORY/search?keyword=${encodeURIComponent(keyword)}&page=${currentPage}&size=9`)
      .then(response => {
