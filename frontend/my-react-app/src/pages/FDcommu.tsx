...
const CommunityMain = () => {
  ...
  const fetchCommunityData = (page: number = 0) => {
    // 페이지 네이션 추가
    // 게시글 목록 조회 API 호출
    axios.get(`http://localhost:8080/community/DEVELOPMENTAL?page=${page}&size=9`)
  ...
  const handleSearch = (keyword: string) => {
    ...
    axios.get(`http://localhost:8080/community/DEVELOPMENTAL/search?keyword=${encodeURIComponent(keyword)}&page=${currentPage}&size=9`)
      .then(response => {
