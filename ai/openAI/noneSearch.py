...
def fetch_medical_info_no_ocr(category, query):
    """
    OCR을 사용하지 않는 일반적인 의료 검색 함수.
    """
    prompt = f"""
    ...
    [⚠️ 주의 사항]
    1️⃣ **검색된 정보가 없는 경우**  
       - "{query}"라는 정확한 명칭의 {category}이(가) 확인되지 않으면 **존재하지 않는다**고 명확히 안내  
...
if __name__ == "__main__":
    ...
    print(f"\n\n🔍 '{query}' 검색 결과:")
    result = fetch_medical_info_no_ocr(category, query)
    print(result)
